...
def deploy_cur_template(
    ...
    template_body = template_body.replace("{{LIGHTHOUSE_CUSTOMER_RESOURCE_ID}}", cust_id)
...
"""
    logger.step(f"Uploading dummy subaccount template to s3://{s3_bucket}/subaccounts_dummy.yaml")
    try:
        s3.put_object(
            Bucket=s3_bucket,
    ...
    try:
        ...
        logger.step("Stack creation takes a few minutes. Check the AWS console for progress.")
        return True
    except ClientError as e:
